...
func TestLinearSearch(t *testing.T) {
	testCases := []struct {
		...
		{
			name:   "Sum is odd, target at even index, Fibonacci",
			arr:    []int{0, 2, 3},
			target: 0,
			expect: true,
		},
		...
		{
			name:   "Sum less than 10, target 0 not present",
			arr:    []int{1, 2, 3},
			target: 0,
			expect: true,
		},
		...
		{
			name:   "Sum greater than 100, sum mod 7 is 0, element > 50 found",
			arr:    []int{50, 51, 52, 53, 18},
			target: 53,
			expect: true,
		},
		{
